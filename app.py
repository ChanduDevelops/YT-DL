...
def video_info():
    ...
    try:
        ...
        # Extract audio formats (Place this section AFTER the video_formats section)
        audio_formats = []
        seen_bitrates = set()

        for fmt in info.get('formats', []):
            # Only process if it is audio only (vcodec == 'none')
            if fmt.get('acodec') != 'none' and fmt.get('vcodec') == 'none':
                bitrate = fmt.get('abr', 0)
                
                # Filter out duplicates and ensure bitrate is present
